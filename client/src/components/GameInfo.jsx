...
const GameInfo = () => {
    ...
    const handleBattleExit = async () => {
        ...
        try {
            ...
            setShowAlert({ status: true, type:'info', message: `你退出了 ${battleName}!`})
        } catch (error) {
        ...
        <>
            ...
            <div className={`${styles.gameInfoSidebar} ${toggleSidebar ? 'translate-x-0': 'translate-x-full'} ${styles.glassEffect} ${styles.flexBetween} backdrop-blur-3xl`}>
                ...
                <div className={`${styles.flexBetween} mt-10 gap-4 w-full`}>
                    <CustomButton title="修改对局背景" handleClick={() => navigate('/battleground')}/>
                    <CustomButton title="退出对局" handleClick={handleBattleExit}/>
                </div>
            </div>
    </>
  )
